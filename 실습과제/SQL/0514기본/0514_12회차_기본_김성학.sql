...
use sqldb;
#usertbl테이블에서 이름이 '김경호'인 행을 출력하시오.
select *
from usertbl
where name = '김경호';
#usertbl테이블에서 생년이 1970이상이고 키가 182이상인 데이터를 출력하세요.
select *
from usertbl
...
#usertbl테이블에서 이름이 '김'으로 시작하는 데이터를 출력하시오.
select *
from usertbl
where name like '김%';
#usertbl에서 김경호 보다 큰 사람들의 이름과 키를 출력하세오.
select name,height
from usertbl5
where height > (select height
                 from usertbl
                 where name='김경호');
#usertbl을 mDate의 오름차순으로 정렬하여 출력하시오.
select *
from usertbl
order by mDate asc;
#usertbl을 mDate의 내림차순으로 정렬하여 출력하시오.
select *
from usertbl
order by mDate desc;
#usertbl을 height의 내림차순으로 정렬하고, 동률인 경우 name의 내림차순으로 정렬하여 출력하시오.
select *
from usertbl
order by height desc, name desc;
#usertbl의 주소지를 중복없이 오름차순으로 출력하시오.
...
#city 테이블에서 국가코드와 인수를 출력하라. 정렬은 국가코드별로 오름차순으로, 동일한 코드(국가)안에서는 인구 수의 역순으로 표시하세요.
